...
if (keepAliveTimeout === undefined || keepAliveTimeout < 0) {
  throw new Error(
    `Invalid timeout configuration: keepAliveTimeout (${keepAliveTimeoutEnv}ms) must be >= 0`,
  );
}
if (headersTimeout === undefined || headersTimeout <= 0) {
  throw new Error(
    `Invalid timeout configuration: headersTimeout (${headersTimeoutEnv}ms) must be > 0`,
  );
}
if (requestTimeout === undefined || requestTimeout <= 0) {
  throw new Error(
    `Invalid timeout configuration: requestTimeout (${requestTimeoutEnv}ms) must be > 0`,
  );
}
